...
export function mountQuiz(root, opts) {
  if (!opts || !opts.content || !opts.state) {
    throw new Error(
      'mountQuiz: main.js에서 fetch("./content/day1.json") 후 { content, state } 로 넘길 것',
    );
  }
  ...
  function tick(now) {
    ...
    if (countdownLeft > 0) {
      ...
      const schedule = advanceQuizSchedule(
        askLeft,
        dt,
        sourceRenderedAt.get(
          content.quizzes[idx] && content.quizzes[idx].sourceMessageIndex,
        ),
        phoneActiveTime,
      );
      ...
      if (
        schedule.shouldOpen ||
        (idx >= content.quizzes.length && askLeft === 0)
      ) {
        openNext();
      }
    }
  ...
  function nextGap() {
    return (
      TUNING.askMinSec + Math.random() * (TUNING.askMaxSec - TUNING.askMinSec)
    );
  }
  ...
  function reset() {
    if (current && current.el.parentNode)
...
function validateContent(content) {
  ...
  content.quizzes.forEach((q, i) => {
    if (!Array.isArray(q.choices) || q.choices.length !== 3) {
      console.error(
        "[quiz] quizzes[" + i + "].choices 는 3개여야 합니다:",
        q.choices,
      );
    }
    if (!Number.isInteger(q.answer) || q.answer < 0 || q.answer > 2) {
      console.error(
        "[quiz] quizzes[" + i + "].answer 는 0..2 여야 합니다:",
        q.answer,
      );
    }
    ...
    if (
      !Number.isInteger(q.sourceMessageIndex) ||
      ...
      throw new Error(
        "[quiz] quizzes[" +
          i +
          "].sourceMessageIndex 는 messages 범위의 정수여야 합니다: " +
          q.sourceMessageIndex,
      );
    }
  });
  ["correct", "wrong", "timeout", "penaltySlow", "penaltyGauge"].forEach(
    (k) => {
...
function showResult(el, content, correct, timedOut, penaltyKind) {
  ...
  el.querySelectorAll(".bm-quiz-choice").forEach((b) => {
    b.disabled = true;
  });
}
